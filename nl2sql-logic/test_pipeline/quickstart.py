...
def check_dataset(dataset_path):
    ...
    required_files = ["dev.json"]
    schema_files = ["tables.json", "dev_tables.json", "test_tables.json"]
    ...
    for filename in schema_files:
        ...
        if file_path.exists():
            size_mb = file_path.stat().st_size / (1024**2)
            print(f"✓ {filename} ({size_mb:.1f} MB)")
            schema_found = filename
            break

    if not schema_found:
        print(f"✗ schema file missing (expected one of: {', '.join(schema_files)})")
        missing_required.append("schema file")
