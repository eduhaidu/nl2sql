...
class PipelineRunner:
    ...
    def __init__(
        ...
        Args:
            ...
            user_id: User id used for dbimport/session tracking
        """
        self.dataset_loader = DatasetLoader(dataset_path, split="dev")
        self.backend_client = BackendClient(backend_url)
        self.test_executor = TestExecutor(self.backend_client, user_id=user_id)
...
def main():
    ...
    parser.add_argument(
        "--user-id",
        default=os.getenv("NL2SQL_USER_ID", "1"),
        help="User id used when creating backend conversations (default: 1)"
    )
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
