...
class DatasetLoader:
    ...
    def __init__(self, dataset_path: str, split: str = "train"):
        ...
        self.database_roots = self._detect_database_roots()
        self.split = split
        self.questions = []
        self.databases = {}

    def _detect_database_roots(self) -> List[Path]:
        """Detect likely database root folders for Spider/Bird layouts."""
        candidates = [
            self.dataset_path / "database",
            self.dataset_path / "dev_databases",
            self.dataset_path / "test_database",
            self.dataset_path / "test_databases",
            self.dataset_path,
        ]

        roots = [candidate for candidate in candidates if candidate.exists()]
        return roots or [self.dataset_path]
    ...
    def load_databases(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        tables_candidates = [
            self.dataset_path / f"{self.split}_tables.json",
            self.dataset_path / "tables.json",
            self.dataset_path / "dev_tables.json",
            self.dataset_path / "test_tables.json",
        ]

        tables_file = next((candidate for candidate in tables_candidates if candidate.exists()), None)
    ...
    def get_database_path(self, db_id: str) -> Path:
        """Return the SQLite file path for a Spider or Bird database ID."""
        if not db_id:
            return None
        ...
        for root in self.database_roots:
            ...
            candidates = [
                db_folder / f"{db_id}.sqlite",
                db_folder / f"{db_id}.db",
                db_folder / f"{db_id}.sqlite3",
                root / f"{db_id}.sqlite",
                root / f"{db_id}.db",
            ...
            if db_folder.exists():
                for extension in ("*.sqlite", "*.db", "*.sqlite3"):
                    matches = list(db_folder.glob(extension))
                    if matches:
                        return matches[0]

            for extension in ("*.sqlite", "*.db", "*.sqlite3"):
                matches = list(root.glob(extension))
                if matches:
                    return matches[0]
